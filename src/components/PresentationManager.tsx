...
export const PresentationManager = () => {
      ...
      <div className="min-h-[calc(100vh-61px)] flex items-center justify-center bg-background">
        <div className="text-center p-4">
          ...
          <p className="text-muted-foreground">
            Select a presentation from the sidebar to manage its slide decks.
          </p>
        </div>
      </div>
    );
  ...
  const handleReorder = (newOrder: SlideDeck[]) => {
    updatePresentation(presentation.id, {
      slideDeckIds: newOrder.map((deck) => deck.id),
    });
  };
  ...
  return (
    <div
      className="min-h-[calc(100vh-101px)] max-w-4xl mx-auto p-4 sm:p-6 md:p-8 space-y-6"
    ...
    >
      <div>
        ...
        <p className="text-lg text-muted-foreground mt-2">
          Drag and drop to reorder slide decks. Select a deck to edit its
          content.
        </p>
      </div>
      ...
      {slideDecks.length > 0 ? (
                ...
                <div className="flex items-center px-4 py-2.5">
                  ...
                  <div className="flex items-center ml-4">
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => setCurrentSlideDeck(deck.id)}
                    >
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="hover:text-destructive dark:hover:bg-destructive/10 hover:bg-destructive/10"
                      onClick={() =>
                        removeSlideDeckFromPresentation(
                          presentation.id,
                          deck.id,
        ...
        <div className="text-center py-16 border-2 border-dashed rounded-lg">
          ...
          <p className="text-muted-foreground mt-2">
            Drag slide decks from the library in the sidebar and drop them onto
            this presentation.
          </p>
        </div>
      )}
